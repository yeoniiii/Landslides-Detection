...
def add_Training (clumpsFile,input_training, config):
    ...
    classesDict = dict()
    classesDict['landslide'] = [1, input_training] # training_labels = config['k_means_segmentation']['input']['training_labels'], input_training = training_labels + filename
    ...
    ratutils.populateClumpsWithClassTraining(clumpsFile, classesDict, tmpPath, # nameFile = (element) + "_segmented", clumpsFile = config['k_means_segmentation']['output']['segments'] + nameFile + '.kea'
                    ...
                    if config['dev_mode']:
                        for filename in os.listdir(training_labels):
                            if filename.endswith(".shp"):
                                ...
                                key_landslide_training = filename[0:filename.__len__()].split('.')[0]
