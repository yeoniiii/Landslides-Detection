...
def merge_segments_no_class(regions,key,config):
    ...
    for region in regions:
        ...
        i = i + 1
    save_segments_in_regions(key, segments_region, config) # region 내의 segment들 저장 (순수 landslide)
    ...
    for region in regions:
        ...
        if class_relabeled_mean == 0:
            ...
            for segment in region:
                ...
                if segment['class'] == 0:
                    list_class_0 = list_class_0.append(single_segment, ignore_index=True) # class 0인 segment append
                else:
                    counter_class_1 = counter_class_1 + 1
                    list_of_series.append(single_segment) # class 1인 segment append
            ...
            if(((counter_class_1)/len(region)) != 0 and (counter_class_1/len(region)) != 1):
                save_wrong_merged(key +'_'+ str(int(time.time())), region, config) # 0 과 1이 섞여있으면 wrong merged
